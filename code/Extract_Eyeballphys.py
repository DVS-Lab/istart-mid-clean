...
lefteye_mask='/data/projects/istart-mid/masks/eyeball_left.nii.gz'
righteye_mask='/data/projects/istart-mid/masks/eyeball_right.nii.gz'

#outdir='/data/projects/istart-mid/derivatives/fsl/EVfiles/sub-1001/mid/run-01_LeftEye.txt'
...
for f in infiles:
    ...
    lefteye_out='/data/projects/istart-mid/derivatives/fsl/EVfiles/sub-%s/%s/run-0%s_LeftEye.txt'%(sub,task,run)
    righteye_out='/data/projects/istart-mid/derivatives/fsl/EVfiles/sub-%s/%s/run-0%s_RightEye.txt'%(sub,task,run)
...
pool = Pool(cores)
results = pool.map(RUN_bash, command_list)
